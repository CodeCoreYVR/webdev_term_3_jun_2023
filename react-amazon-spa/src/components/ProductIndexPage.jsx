...
export default class ProductIndexPage extends Component {
  ...
  handleCreate(params) {
    ...
    this.setState((prevState) => {
      ...
      return { products: [{ ...params, id: maxId +1 }, ...prevState.products] }
    })
  }
          ...
          {this.state.loading ? (
            ...
            <>
              {products.map(product => {
                ...
                return (
                  <li className="list-group-item d-flex justify-content-between align-items-center mb-1" key={product.id}>
                    <div>
                      {product.id} -{" "}
                      <Link to={`/products/${product.id}`} className="no-underline">{product.title}</Link>{" "}
                    </div>
                    <div>
                      <button className="btn btn-secondary btn-sm" onClick={() => this.handleDelete(product.id)}>Delete</button>{" "}
                    </div>
                  </li>
